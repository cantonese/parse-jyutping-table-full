...
function cachedLoad() {
  if (fs.existsSync('./JPTableFull.json')) {
    if (fs.existsSync('./output.json')) {
      console.log('Regenerating output.json');
    } else {
      console.log('Generating output.json');
    }
    var pdfData = JSON.parse(fs.readFileSync('./JPTableFull.json'));
    var resultJSONString = main(pdfData.formImage.Pages);
    fs.writeFileSync('output.json', resultJSONString);
    console.log('COMPLETE! Extracted data in output.json');
